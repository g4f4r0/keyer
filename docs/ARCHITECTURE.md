...
meeting audio → SpeechTranscriptionProvider → TextGenerationProvider → iCloud Markdown
```
...
The OpenRouter API key is stored as a synchronizable Keychain password. Portable preferences use iCloud key-value storage; hardware-specific preferences remain local. Speech and text models are selected independently.
...
- meeting audio remains available for retry until transcription, summary, and iCloud staging succeed

## Storage

Every successful dictation or meeting becomes one Markdown file in iCloud Drive with YAML front matter. A local pending copy survives iCloud outages and is removed after coordinated synchronization. Dictation audio is not archived. Failed meeting audio remains temporary only while retry is available.
